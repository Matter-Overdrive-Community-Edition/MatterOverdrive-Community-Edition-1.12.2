...
public class MOAndroidHouseBuilding extends MOWorldGenBuilding {
    ...
    public boolean isFlat(World world, BlockPos pos) {
        ...
        if (Math.abs(pos.getY() - y10.getY()) <= airLeeway && Math.abs(pos.getY() - y11.getY()) <= airLeeway && Math.abs(pos.getY() - y01.getY()) <= airLeeway) {
            return blockBelowMatches(airLeeway, world, Blocks.GRASS, pos) && blockBelowMatches(airLeeway, world, Blocks.GRASS, pos.add(layerWidth, 0, 0)) && blockBelowMatches(airLeeway, world, Blocks.GRASS, pos.add(0, 0, layerHeight)) && blockBelowMatches(airLeeway, world, Blocks.GRASS, pos.add(layerWidth, 0, layerHeight));
        }
        return false;
    }
