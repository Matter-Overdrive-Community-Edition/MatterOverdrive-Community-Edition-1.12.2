...
public class TileEntityGravitationalAnomaly extends MOTileEntity implements IScannable, IMOTickable,IGravitationalAnomaly
{
	...
	private boolean intersectsAnomaly(Vector3 entityPos2, Vector3 dir2, Vector3 blockPos2, double eventHorizon) {
		if (entityPos2.distanceTo(blockPos2) <= eventHorizon) {
			return true;
		} else {
			Vector3 intersectDir = entityPos2.subtract(blockPos2);
			double c = intersectDir.length();
			double v = intersectDir.dotProduct(dir);
			double d = eventHorizon * eventHorizon - (c * c - v * v);
    ...
    private Vector3 normalize(Vector3 dir2)
    {
        double d0 = (double) MathHelper.sqrt(dir2.x * dir2.x + dir2.y * dir2.y + dir2.z * dir2.z);
        if (d0 < 1.0E-4D)
        {
            dir2.x = 0;
            dir2.y = 0;
		...
		if (blockDestoryTimer.hasDelayPassed(world, BLOCK_DESTORY_DELAY)) {
			for (int x = -range; x < range; x++)
			{
				for (int y = -range; y < range; y++)
				{
					for (int z = -range; z < range; z++)
					{
						blockPosX = getPos().getX() + x;
                        blockPosY = getPos().getY() + y;
                        blockPosZ = getPos().getZ() + z;
						blockPos = new BlockPos(getPos().getX() + x, getPos().getY() + y, getPos().getZ() + z);
						blockState = world.getBlockState(blockPos);
						//distance = Math.sqrt(blockPos.distanceSq(getPos()));
						distance = MOMathHelper.distance(blockPosX,blockPosY,blockPosZ,getPos().getX(),getPos().getY(),getPos().getZ());
						hardness = blockState.getBlockHardness(world, blockPos);
						if (blockState.getBlock() instanceof IFluidBlock
								|| blockState.getBlock() instanceof BlockLiquid) {
							hardness = 1;
						...
						float strength = getBreakStrength((float) distance / 3, range);
                        blockState.getBlock();
                        if (blockState.getBlock() != Blocks.AIR && distance <= range && hardness >= 0 && hardness <= strength && distance <= eventHorizon) {
                        	System.out.println("manageBlockDestory Block: " + blockState.getBlock() + " Hardness: " + hardness + " strength: " + strength + " Distance: " + distance + " Horizon: " + eventHorizon);
							blocks.add(blockPos);
						}
					}
				}
			...
			if (!cleanFlowingLiquids(blockState, position)) {
				...
				if (solidCount < MAX_BLOCKS_PER_HARVEST) {
					try {
						...
						float strength = getBreakStrength((float) distance, range);
						//if (breakBlock(world, position, strength, eventHorizon, range)) {
if (brakeBlock(world, position.getX(), position.getY(), position.getZ(), strength, eventHorizon, range)) {
							solidCount++;
						}
					} catch (Exception e) {
						MOLog.log(Level.ERROR, e, "There was a problem while trying to brake block %s",
	...
	public boolean brakeBlock(World world,int x,int y,int z,float strength,double eventHorizon,int range) {
		...
		float hardness = blockState.getBlockHardness(world, pos);
		//double distance = Math.sqrt(pos.distanceSq(getPos()));
		double distance = MOMathHelper.distance(x,y,z,pos.getX(),pos.getY(),pos.getZ());
	////////////////////////////////////////////////////////////////////////////////////////////////////////////	MOMathHelper.randomSpherePoint(getPos().getX()
		
		if (distance <= range && hardness >= 0 && hardness <= strength && distance <= eventHorizon) {
			//System.out.println("breakBlock Block: " + blockState.getBlock() + " Hardness: " + hardness + " strength: " + strength );
			System.out.println("breakBlocky Block: " + blockState.getBlock() + " Hardness: " + hardness + " strength: " + strength + " Distance: " + distance + " Horizon: " + eventHorizon);
			if (BLOCK_ENTETIES) {
