...
public class TileEntityNetworkPipe extends TileEntityPipe implements IMatterNetworkConnection, IEnergyStorage {
		...
		if (!world.isRemote) {
			...
			for (EnumFacing enumFacing : EnumFacing.VALUES) {
				...
				if (tileEntityNeignbor instanceof IMatterNetworkConnection) {
					if (connectionCount < 2 && ((IMatterNetworkConnection) tileEntityNeignbor)
			...
			for (EnumFacing enumFacing : EnumFacing.VALUES) {
				if (isConnectableSide(enumFacing)) {
					...
					if (tileEntityConnection instanceof IMatterNetworkConnection) {
						((IMatterNetworkConnection) tileEntityConnection).breakConnection(state,
	 ...
	 * 
	 * @Override public void broadcast(MatterNetworkPacket packet,EnumFacing
	 * direction) { if (isValid()) { for (int i = 0; i < 6; i++) { if
	 * (direction.getOpposite().ordinal() != i)
	 * MatterNetworkHelper.broadcastPacketInDirection(world, packet, this,
	 * EnumFacing.VALUES[i]); } } }
	 */
